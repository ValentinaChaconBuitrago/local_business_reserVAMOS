...
function crearRestaurates() {
  lista = [];
  lista.forEach((res) => {
    agregarRestaurante(res);
  });
}
function trearRest() {
  mu.connect()
...
function generarRes(res) {
  let lista = [];
  lista = res;
  let fecha = "";
  let nPersonas = 0;
  lista.forEach((element) => {
    disponibles(element._id, fecha, nPersonas, element.nMax);
  });
}
const disponibles = (id, fecha, nPersonas, nMax) => {
  let horas = [
    "12:00",
    "13:00",
    "14:00",
  ...
  mu.connect()
    ...
    .then((user) => {
      ...
      console.log(respu);
      crearReserva(id, fecha, nPersonas, respu);
    })

    .catch((err) => console.log(err));
};
function crearReserva(id, fecha, nPersonas, respu) {}
